...
function lanip()
	local lan_int_name = uci:get("clashnivo", "@overwrite[0]", "lan_interface_name") or uci:get("clashnivo", "config", "lan_interface_name") or "0"
	local lan_ip
...
function uci_get_config(section, key)
	...
	if section == "config" then
    	val = uci:get("clashnivo", "@overwrite[0]", key)
	end
    if val == nil then
    	val = uci:get("clashnivo", section, key)
    end
    return val
end