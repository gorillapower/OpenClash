...
set_lock() {
   exec 876>"/tmp/lock/clashnivo_groups_get.lock" 2>/dev/null
   flock -x 876 2>/dev/null
}

del_lock() {
   flock -u 876 2>/dev/null
   rm -rf "/tmp/lock/clashnivo_groups_get.lock"
}
...
del_options()
{
   ...
   if [ "$config" = "$CONFIG_NAME" ]; then
      uci -q delete clashnivo."$section"
   fi
}
...
ruby -ryaml -rYAML -I "/usr/share/clashnivo" -E UTF-8 -e "
   ...
   Value['proxy-groups'].each_with_index do |x, index|
      uci_name_tmp << %x{uci -q add clashnivo groups 2>&1}.chomp
      queue.push(nil)
      threadsp << Thread.new {
      begin
...
/usr/share/clashnivo/yml_proxys_get.sh "$CONFIG_FILE" >/dev/null 2>&1
del_lock