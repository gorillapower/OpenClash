...
del_lock() {
   ...
   rm -rf "/tmp/lock/clashnivo_update.lock" 2>/dev/null
}
...
if [ -n "$1" ] && [ "$1" != "one_key_update" ]; then
   /usr/share/clashnivo/openclash_version.sh "$1" 2>/dev/null
elif [ -n "$2" ]; then
   /usr/share/clashnivo/openclash_version.sh "$2" 2>/dev/null
else
   /usr/share/clashnivo/openclash_version.sh 2>/dev/null
fi
...
LAST_OPVER="/tmp/clashnivo_last_version"
LAST_VER=$(sed -n 1p "$LAST_OPVER" 2>/dev/null |sed "s/^v//g" |tr -d "\n")
if [ -x "/bin/opkg" ]; then
   OP_CV=$(rm -f /var/lock/opkg.lock && opkg status luci-app-clashnivo 2>/dev/null |grep 'Version' |awk -F 'Version: ' '{print $2}' 2>/dev/null)
...
if [ "$1" = "one_key_update" ]; then
   ...
   if [ -n "$2" ]; then
      /usr/share/clashnivo/openclash_core.sh "Meta" "$1" "$2" >/dev/null 2>&1 &
      github_address_mod="$2"
   else
      /usr/share/clashnivo/openclash_core.sh "Meta" "$1" >/dev/null 2>&1 &
   ...
   while [ "$retry_count" -lt "$max_retries" ]; do
      ...
      if [ "$DOWNLOAD_RESULT" -ne 1 ]; then
         ...
         if [ -x "/bin/opkg" ]; then
            ...
            if [ -s "/tmp/clashnivo.ipk" ]; then
               if [ -n "$(opkg install /tmp/clashnivo.ipk --noaction 2>/dev/null |grep 'Upgrading luci-app-clashnivo on root' 2>/dev/null)" ]; then
            ...
            if [ -s "/tmp/clashnivo.apk" ]; then
               apk add -s -q --force-overwrite --clean-protected --allow-untrusted /tmp/clashnivo.apk >/dev/null 2>&1
               if [ $? -eq 0 ]; then
                  pre_test_success=true
               fi
            fi
...
check_install_success()
{
   ...
   if [ -x "/bin/opkg" ]; then
      current_version=$(rm -f /var/lock/opkg.lock && opkg status luci-app-clashnivo 2>/dev/null |grep 'Version' |awk -F 'Version: ' '{print $2}' 2>/dev/null)
   elif [ -x "/usr/bin/apk" ]; then
      current_version=$(apk list luci-app-clashnivo 2>/dev/null|grep 'installed' | grep -oE '[0-9]+(\.[0-9]+)*' | head -1 2>/dev/null)
   fi
...
while [ $install_retry_count -lt $max_install_retries ]; do
   ...
   if [ -x "/bin/opkg" ]; then
      ...
      opkg install /tmp/clashnivo.ipk
   elif [ -x "/usr/bin/apk" ]; then
      for pkg in $packages_to_check; do
         if apk info "$pkg" >/dev/null 2>&1; then
            installed_before="$installed_before $pkg"
      ...
      apk add -q --force-overwrite --clean-protected --allow-untrusted /tmp/clashnivo.apk
   fi
...
EOF
   ...
   while [ $retry_count -lt $max_retries ]; do
      ...
      ubus call service add '{"name":"openclash_update","instances":{"update":{"command":["/tmp/clashnivo_update.sh"],"stdout":true,"stderr":true,"env":{"LAST_VER":"'"$LAST_VER"'"}}}}' >/dev/null 2>&1
