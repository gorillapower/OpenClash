...
set_lock() {
   exec 882>"/tmp/lock/clashnivo_name_ch.lock" 2>/dev/null
   flock -x 882 2>/dev/null
}

del_lock() {
   flock -u 882 2>/dev/null
   rm -rf "/tmp/lock/clashnivo_name_ch.lock"
}
...
cfg_groups_set()
{
   ...
   if [ -z "$old_name_cfg" ]; then
      uci -q set clashnivo."$section".old_name_cfg="$name"
      uci -q commit clashnivo
   fi

   if [ -z "$old_name" ]; then
      uci -q set clashnivo."$section".old_name="$name"
      uci -q commit clashnivo
   fi
