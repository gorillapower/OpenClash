   ...
   if Value.key?('proxy-providers') and not Value['proxy-providers'].nil?
      Value['proxy-providers'].each do |name, provider|
         if provider.key?('path') and not provider['path'].empty?
            path = provider['path'].start_with?('./') ? '/etc/clashnivo/' + provider['path'][2..-1] : provider['path']
            if File.exist?(path)
               begin
                  ...
                  begin
                     syscall = \"lua /usr/share/clashnivo/openclash_sub_parser.lua \\\"#{path}\\\"\"
      ...
      threads = (1..[10, queue.size].min).map do
         Thread.new do
            while domain = queue.pop(true) rescue nil
               syscall = '/usr/share/clashnivo/openclash_debug_dns.lua 2>/dev/null \"' + domain + '\" \"true\"'
...
## Log File Size Manage:
   LOGSIZE=`ls -l /tmp/clashnivo.log |awk '{print int($5/1024)}'`
...
## UPNP
   if [ -f "$upnp_lease_file" ]; then
      #del
      if [ -n "$FW4" ]; then
         for i in `$(nft list chain inet fw4 clashnivo_upnp |grep "return")`
         do
         ...
         for i in `$(iptables -t mangle -nL clashnivo_upnp |grep "RETURN")`
         do
...
## 配置文件循环更新
   if [ "$cfg_update" -eq 1 ] && [ "$cfg_update_mode" -eq 1 ]; then
      ...
      if [ "$CFG_UPDATE_INT" -ne 0 ]; then
         [ "$(expr "$CFG_UPDATE_INT" % "$cfg_update_interval_now")" -eq 0 ] && /usr/share/clashnivo/openclash.sh
      fi
