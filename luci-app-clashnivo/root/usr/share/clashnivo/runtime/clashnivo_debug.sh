...
set_lock() {
   exec 885>"/tmp/lock/clashnivo_debug.lock" 2>/dev/null
   flock -x 885 2>/dev/null
}

del_lock() {
   flock -u 885 2>/dev/null
   rm -rf "/tmp/lock/clashnivo_debug.lock" 2>/dev/null
}
...
DEBUG_LOG="/tmp/clashnivo_debug.log"
...
core_meta_version=$(/etc/clashnivo/core/clash_meta -v 2>/dev/null |awk -F ' ' '{print $3}' |head -1 2>/dev/null)
op_version=$(ipk_v "luci-app-clashnivo")
...
运行内核：$core_type
进程pid: $(pidof clash)
运行用户: $(ps |grep "/etc/clashnivo/clash" |grep -v grep |awk '{print $2}' 2>/dev/null)
EOF
else
cat >> "$DEBUG_LOG" <<-EOF
运行状态: 未运行
...
if [ ! -f "/etc/clashnivo/core/clash_meta" ]; then
cat >> "$DEBUG_LOG" <<-EOF
Meta内核文件: 不存在
EOF
else
...
if [ ! -x "/etc/clashnivo/core/clash_meta" ]; then
cat >> "$DEBUG_LOG" <<-EOF
Meta内核运行权限: 否
EOF
else
...
$(uci -q show clashnivo.@overwrite[0])
...
#===================== 自定义规则 一 =====================#
EOF
cat /etc/clashnivo/custom/clashnivo_custom_rules.list >> "$DEBUG_LOG"

cat >> "$DEBUG_LOG" <<-EOF

#===================== 自定义规则 二 =====================#
EOF
cat /etc/clashnivo/custom/clashnivo_custom_rules_2.list >> "$DEBUG_LOG"
fi
...
cat /etc/clashnivo/custom/clashnivo_custom_overwrite.sh >> "$DEBUG_LOG" 2>/dev/null
...
cat /etc/clashnivo/custom/clashnivo_custom_firewall_rules.sh >> "$DEBUG_LOG" 2>/dev/null
...
EOF
/usr/share/clashnivo/clashnivo_debug_dns.lua "www.instagram.com" >> "$DEBUG_LOG" 2>/dev/null
...
EOF
VERSION_URL="https://raw.githubusercontent.com/openwrt/openwrt/main/README.md"
if pidof clash >/dev/null; then
...
tail -n 100 "/tmp/clashnivo.log" >> "$DEBUG_LOG" 2>/dev/null
cat >> "$DEBUG_LOG" <<-EOF
...
EOF
/usr/share/clashnivo/clashnivo_debug_getcon.lua
