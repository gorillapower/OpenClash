         ...
         if provider.key?('path') and not provider['path'].empty?
            ...
            if File.exist?(path)
               begin
                  ...
                  begin
                     syscall = \"lua /usr/share/clashnivo/clashnivo_sub_parser.lua \\\"#{path}\\\"\"
      ...
      threads = (1..[10, queue.size].min).map do
         Thread.new do
            while domain = queue.pop(true) rescue nil
               syscall = '/usr/share/clashnivo/clashnivo_debug_dns.lua 2>/dev/null \"' + domain + '\" \"true\"'
...
## 配置文件循环更新
   if [ "$cfg_update" -eq 1 ] && [ "$cfg_update_mode" -eq 1 ]; then
      ...
      if [ "$CFG_UPDATE_INT" -ne 0 ]; then
         [ "$(expr "$CFG_UPDATE_INT" % "$cfg_update_interval_now")" -eq 0 ] && /usr/share/clashnivo/clashnivo_refresh.sh
      fi
